...
class ThreeJSScene {
    ...
    init() {
        ...
        // Проверяем создание renderer
        if (!this.createRenderer()) {
            console.warn('Three.js renderer не создан, останавливаем инициализацию');
            return;
        }
    ...
    createRenderer() {
        ...
        if (!canvas) {
            console.warn('Canvas для Three.js не найден, пропускаем инициализацию');
            return false;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    // Проверяем доступность THREE и canvas
    if (typeof THREE === 'undefined') {
        console.warn('THREE.js не загружен, пропускаем инициализацию 3D сцены');
        return;
    }
    
    const canvas = document.getElementById('threejs-canvas');
    if (!canvas) {
        console.warn('Canvas для Three.js не найден, пропускаем инициализацию 3D сцены');
        return;
    }
