...
// Отправка заказа на email через EmailJS + наш сервер
function sendOrderEmail(order) {
    ...
    // Отправляем на сервер для логирования и бэкапа
    ...
    if (typeof emailjs !== 'undefined') {
        emailjs.send('service_6ogl5un', 'template_9012236', templateParams)
            ...
            .catch(function(error) {
                console.log('❌ Ошибка EmailJS, но заказ сохранен на сервере:', error);
                showSuccessMessage(); // Все равно показываем успех, т.к. сервер сохранил
            });
    } else {
        console.log('❌ EmailJS не загружен, но заказ отправлен на сервер');
        showSuccessMessage();
    }
}
...
document.getElementById('testimonialForm').addEventListener('submit', function(event) {
    ...
    sendReviewToServer(review).then(response => {
        if (response && response.approved) {
            // Если отзыв сразу одобрен (было разрешение), перезагружаем отображение
            setTimeout(() => {
                loadReviews();
            }, 1000);
        }
...
function sendReviewNotification(review) {
    ...
    // Отправляем через EmailJS если доступен
    if (typeof emailjs !== 'undefined') {
        emailjs.send('service_6ogl5un', 'template_9012236', templateParams)
            .then(function(response) {
        ...
        console.log('✅ Отзыв сохранен на сервере');
    }
}

// Загрузка и отображение отзывов
function loadReviews() {
    // Сначала загружаем с сервера
    fetch('/api/reviews')
        .then(response => response.json())
        .then(serverReviews => {
            // Фильтруем только одобренные отзывы с сервера
            const approvedServerReviews = serverReviews.filter(review => review.permission || review.approved);
            
            // Загружаем отзывы из localStorage для совместимости
            const localReviews = JSON.parse(localStorage.getItem('reviews') || '[]');
            const approvedLocalReviews = localReviews.filter(review => review.approved);
            
            // Объединяем отзывы (серверные имеют приоритет)
            const allApprovedReviews = [...approvedServerReviews, ...approvedLocalReviews];
            
            displayReviews(allApprovedReviews);
        })
        .catch(error => {
            console.log('❌ Ошибка загрузки отзывов с сервера, используем localStorage:', error);
    ...
    reviewsGrid.innerHTML = reviews.map(review => `
        <div class="review-card fade-in">
            <div class="review-header">
                ...
                <div class="review-date">${review.date || new Date(review.timestamp).toLocaleDateString('ru-RU')}</div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        showNotification('👋 Добро пожаловать в Образовательный Хаб!', 'info');
    }, 1000);
});
...
function updateOrderSummary() {
    ...
    const projectTypeText = document.getElementById('orderProjectType').options[document.getElementById('orderProjectType').selectedIndex].text;
...
document.getElementById('orderForm').addEventListener('submit', function(event) {
    ...
    const order = {
        ...
        projectType: document.getElementById('orderProjectType').value,
...
function approveReview(reviewId) {
    console.log(`🔧 Пытаемся одобрить отзыв с ID: ${reviewId}`);
    ...
    if (reviewIndex !== -1) {
        ...
        localStorage.setItem('reviews', JSON.stringify(reviews));
        console.log('✅ Отзыв одобрен в localStorage');
        showNotification('✅ Отзыв одобрен и опубликован!', 'success');
    } else {
        console.log('⚠️ Отзыв не найден в localStorage');
    }
    
    // Отправляем одобрение на сервер
    console.log('📤 Отправляем одобрение на сервер...');
    approveReviewOnServer(reviewId).then(() => {
        console.log('🔄 Перезагружаем отзывы на странице...');
        // После успешного одобрения на сервере, перезагружаем отзывы
        loadReviews();
    }).catch(error => {
        console.log('❌ Ошибка при одобрении на сервере:', error);
    });
}
...
// Тестовая функция для проверки работы approveReview
function testApproveReview() {
    console.log('🧪 Тестирование функции approveReview...');
    
    // Сначала смотрим отзывы
    viewAllReviews();
    
    // Через 2 секунды пробуем одобрить первый найденный отзыв
    setTimeout(() => {
        fetch('/api/reviews')
            .then(response => response.json())
            .then(reviews => {
                const unapprovedReview = reviews.find(review => !review.approved);
                if (unapprovedReview) {
                    console.log(`🎯 Найден неодобренный отзыв: ${unapprovedReview.id}`);
                    console.log(`🧪 Пробуем одобрить: approveReview(${unapprovedReview.id})`);
                    approveReview(unapprovedReview.id);
                } else {
                    console.log('ℹ️ Все отзывы уже одобрены');
                }
            })
            .catch(error => {
...
function sendReviewToServer(review) {
    ...
    .then(data => {
        if (data.success) {
            ...
            console.log(`📊 Статус: ${data.approved ? 'Опубликован' : 'Ожидает модерации'}`);
            return data;
        } else {
...
// Функция для проверки версии и обновления при необходимости
function checkVersion() {
    ...
    if (storedVersion !== currentVersion) {
        ...
        showNotification('🔄 Сайт обновлен до новой версии!', 'info');
...
document.addEventListener('DOMContentLoaded', function() {
    // Отключаем проверку версии на время отладки
    // checkVersion();
    
    // Принудительная очистка кэша для Three.js
    if ('caches' in window) {
        caches.keys().then(function(names) {
            names.forEach(function(name) {
                if (name.includes('threejs-scene') || name.includes('three.min')) {
                    caches.delete(name);
                }
            });
        });
