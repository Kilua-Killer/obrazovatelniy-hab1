...
class ScrollController {
    ...
    animate() {
        ...
        // Плавная прокрутка с оптимизацией
...
function toggleMobileMenu() {
    console.log('Toggle mobile menu called'); // Отладка
    const navLinks = document.getElementById('navLinks');
    if (navLinks) {
        navLinks.classList.toggle('active');
        console.log('Menu toggled, classes:', navLinks.className); // Отладка
        console.log('Menu display style:', window.getComputedStyle(navLinks).display); // Отладка
    } else {
        console.log('navLinks element not found'); // Отладка
    }
}

function closeMobileMenu() {
    console.log('Close mobile menu called'); // Отладка
    const navLinks = document.getElementById('navLinks');
    if (navLinks) {
        navLinks.classList.remove('active');
        console.log('Menu closed'); // Отладка
    }
}

// Альтернативный обработчик для мобильных устройств
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM loaded, setting up mobile menu'); // Отладка
    
    const mobileToggle = document.querySelector('.mobile-menu-toggle');
    const navLinks = document.getElementById('navLinks');
    
    console.log('Mobile toggle found:', !!mobileToggle); // Отладка
    console.log('Nav links found:', !!navLinks); // Отладка
    
    if (mobileToggle && navLinks) {
        // Добавляем обработчики для разных типов событий
        mobileToggle.addEventListener('click', function(e) {
            e.preventDefault();
            console.log('Click event triggered'); // Отладка
            toggleMobileMenu();
        });
        
        mobileToggle.addEventListener('touchstart', function(e) {
            e.preventDefault();
            console.log('Touch event triggered'); // Отладка
            toggleMobileMenu();
        });
        ...
        menuItems.forEach(item => {
            item.addEventListener('click', function() {
                console.log('Menu item clicked, closing menu'); // Отладка
                closeMobileMenu();
            });
        });
        
        console.log('Mobile menu setup complete'); // Отладка
    } else {
        console.log('Mobile menu elements not found'); // Отладка
    }
});
...
document.querySelectorAll('.star').forEach(star => {
    star.addEventListener('click', function() {
        ...
        for (let i = 1; i <= rating; i++) {
            document.querySelector(`.star[data-rating="${i}"]`).style.opacity = '1';
        }
    });
});
...
// Функция для одобрения отзывов (для админа)
