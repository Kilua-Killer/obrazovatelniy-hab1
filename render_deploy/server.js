...
// Загружаем конфигурацию email
const EMAIL_CONFIG = {
  user: 'obrazovatelniyhab@yandex.ru',
  pass: 'cyjmrogbdkmcmbxl', // Пароль приложения Yandex
  host: 'smtp.yandex.ru',
  port: 465
};
...
const server = http.createServer((req, res) => {
  ...
  // Определяем MIME тип ПОСЛЕ проверки файла
  ...
  console.log(`📋 Расширение: ${extname}, MIME тип: ${contentType}`);
...
function handleOrder(order, res) {
  ...
  sendOrderEmail(orderWithTimestamp);
...
// Функция отправки email уведомления о заказе
async function sendOrderEmail(order) {
  try {
    // Создаем транспорт для отправки email через Yandex
    ...
    const mailOptions = {
      ...
      subject: `🔔 Новый заказ: ${order.projectType}`,
      html: `
        <h2>🔔 Новый заказ на сайте Образовательный Хаб!</h2>
        <p><strong>📅 Дата:</strong> ${new Date(order.timestamp).toLocaleString('ru-RU')}</p>
        <p><strong>👤 Имя:</strong> ${order.name}</p>
        <p><strong>📧 Email:</strong> ${order.email}</p>
        <p><strong>📞 Телефон:</strong> ${order.phone}</p>
        <p><strong>📋 Проект:</strong> ${order.projectType}</p>
        ${order.projectName ? `<p><strong>📝 Название проекта:</strong> ${order.projectName}</p>` : ''}
        <p><strong>💰 Цена:</strong> ${order.price || 'Не рассчитана'} ₽</p>
        <p><strong>💳 Способ оплаты:</strong> ${order.paymentMethod || 'Не указан'}</p>
        <p><strong>📝 Описание:</strong></p>
        <p>${order.description}</p>
        <hr>
        <p><small>Это автоматическое уведомление с сайта Образовательный Хаб</small></p>
      `
    };

    // Отправляем письмо
    const info = await transporter.sendMail(mailOptions);
    console.log('✅ Email уведомление отправлено:', info.messageId);
